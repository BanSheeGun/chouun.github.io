...
namespace csl {
  ...
  std::pair<int,point> operator &(const line& a,const line& b) {
    if(sgn((a.s - a.e) ^ (b.s - b.e)) == 0) {
      if(sgn((a.s - b.e) ^ (b.s - b.e)) == 0) return std::make_pair(0, point());
      else return std::make_pair(1, point());
    }
    double t = ((a.s - b.s) ^ (b.s - b.e)) / ((a.s - a.e) ^ (b.s - b.e));
    return std::make_pair(2,
        point(a.s.x + (a.e.x - a.s.x) * t, a.s.y + (a.e.y - a.s.y) * t));
  }
  ...
  point p2seg(const point& p,const line& l) {
    double t = ((p - l.s) * (l.e - l.s)) / ((l.e - l.s) * (l.e - l.s));
    if(t >= 0 && t <= 1) return point(l.s.x + (l.e.x - l.s.x) * t,
        l.s.y + (l.e.y - l.s.y) * t);
  ...
  bool graham_cmp(point p1,point p2) {
    ...
    else if(sgn(tmp) == 0 && sgn(dist(p1, graham_T) - dist(p2, graham_T)) <= 0) return true;
    else return false;
  }
