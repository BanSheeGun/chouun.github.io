  ...
  public : 
    ...
    operator -= (const BigInt& b)
    {
      ...
      for (int i = 0; i < m_size; i++)
      {
        ...
        if (m_data[i] < 0) c = 1, m_data[i] += BIGINT_BASE;
        else c = 0;
      }
      while (m_size > 1 && !m_data[m_size - 1]) m_size--;
      return *this;
    ...
    {
      int i = 0;
      if (!b) return *this = BigInt();
      for (long long c = 0; i < _size || c > 0; i++)
      {
        if (i < _size) c += (long long)(_data[i]) * b;
        _data[i] = c % BIGINT_BASE;
        c /= BIGINT_BASE;
      }
      _size = i;
      return *this;
    ...
    operator /= (const int b)
    {
      ...
      {
        c = c * BIGINT_BASE + _data[i];
        r._data[i] = c / b;
        c %= b;
      }
      memcpy(_data, r._data, sizeof(int) * _size);
    ...
    friend std::istream&
    operator >> (std::istream &i, BigInt& n)
    {
      std::string s;
      i >> s;
      n = BigInt(s.c_str());
    ...
    friend std::ostream&
    operator << (std::ostream &o, const BigInt& n)
    {
