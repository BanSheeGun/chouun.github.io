...
namespace csl
{
  ...
  //@ 判断线段与线段相交
  bool
  inter_ss(const line& a, const line& b)
  {
    return
        ...
        sgn((b.s - a.e) ^ (a.s - a.e)) *
        sgn((b.e - a.e) ^ (a.s - a.e)) <= 0 &&
        sgn((a.s - b.e) ^ (b.s - b.e)) *
        sgn((a.e - b.e) ^ (b.s - b.e)) <= 0;
  }

  //@ 判断直线和线段相交
  inline bool
  inter_ls(const line& a, const line& b)
  {
    return
        sgn((b.s - a.e) ^ (a.s - a.e)) *
        sgn((b.e - a.e) ^ (a.s - a.e)) <= 0;
  ...
  //@ 点到线段最近的点
  point
  p2seg(const point& p, const line& l)
  {
    double t = ((p - l.s) * (l.e - l.s)) / ((l.e - l.s) * (l.e - l.s));
    if (t >= 0 && t <= 1)
      return point( l.s.x + (l.e.x - l.s.x) * t, l.s.y + (l.e.y - l.s.y) * t);
  ...
  //@ 判断点在线段上
  inline bool
  ponseg(const point& p, const line& l)
  {
    return
        sgn((l.s - p) ^ (l.e - p)) == 0 &&
  ...
  //@ -1为外,0为上,1为内
  int
  pincploy(point x, const point p[], int n)
  {
    for (int i = 0; i < n; ++i)
    {
      if (sgn((p[i] - x) ^ (p[(i+1)%n] - x)) < 0)
        return -1;
      if (ponseg(x, line(p[i], p[(i+1)%n])))
        return 0;
    }
    return 1;
  }
  ...
  //@ -1为外,0为上,1为内
  int
  pinploy(point x, const point p[], int n)
  {
