...
namespace csl
{
  struct sb_tree_node_base
  {
    ...
    static inline _Base_ptr
    s_minimum(_Base_ptr __x)
    {
      while (__x->m_left) __x = __x->m_left;
      return __x;
    }

    static inline _Const_Base_ptr
    s_minimum(_Const_Base_ptr __x)
    {
      while (__x->m_left) __x = __x->m_left;
      return __x;
    }
    ...
    static inline _Base_ptr
    s_maximum(_Base_ptr __x)
    {
      while (__x->m_right) __x = __x->m_right;
      return __x;
    }

    static inline _Const_Base_ptr
    s_maximum(_Const_Base_ptr __x)
    {
      while (__x->m_right) __x = __x->m_right;
      return __x;
    }
  ...
  struct sb_tree_node : public sb_tree_node_base
  {
    ...
    sb_tree_node(_Tp __x) : m_value(__x) { }
  };

  inline void
  sb_tree_left_rotate(sb_tree_node_base*& __x)
  {
  ...
  inline void
  sb_tree_right_rotate(sb_tree_node_base*& __x)
  {
  ...
  void
  sb_tree_maintain(sb_tree_node_base* __x, bool __f)
  {
    if (__x == 0) return;
    if (__f == true)
    {
      std::size_t __l =  __x->m_left ? __x->m_left->m_size : 0;
      std::size_t __rl =  __x->m_right ? __x->m_right->m_left ?
          __x->m_right->m_left->m_size : 0 : 0;
      std::size_t __rr =  __x->m_right ? __x->m_right->m_right ?
          __x->m_right->m_right->m_size : 0 : 0;
      if (__rr > __l)
        sb_tree_left_rotate(__x);
      else if (__rl > __l)
        sb_tree_right_rotate(__x->m_right),
        sb_tree_left_rotate(__x);
      else
        return;
    }
    else
    {
      ...
      std::size_t __ll =  __x->m_left ? __x->m_left->m_left ?
          __x->m_left->m_left->m_size : 0 : 0;
      std::size_t __lr =  __x->m_left ? __x->m_left->m_right ?
          __x->m_left->m_right->m_size : 0 : 0;
      if (__ll > __r)
        sb_tree_right_rotate(__x);
      else if (__lr > __r)
        sb_tree_left_rotate(__x->m_left),
        sb_tree_right_rotate(__x);
      else
        return;
    }
  ...
  {
    if (__x->m_right)
      __x = sb_tree_node_base::s_minimum(__x->m_right);
    else
    {
  ...
  {
    if (__x->m_parent->m_parent == __x)
      __x = __x->m_right;
    else if (__x->m_left)
      __x = sb_tree_node_base::s_maximum(__x->m_left);
    else
    {
      sb_tree_node_base* __y = __x->m_parent;
  ...
  public :
    ...
    explicit
    sb_tree_iterator(_Link_type __x)
    : m_node(__x) { }
  ...
  private : 
    // member function.
    void
    s_insert(_Base_ptr& __x, _Base_ptr __y, const value_type& __v)
    {
      if (__x == 0)
      {
        __x = new _Node(__v);
      ...
      {
        __x->m_size += 1;
        if (__v < static_cast<_Link_type>(__x)->m_value)
          s_insert(__x->m_left, __x, __v),
          sb_tree_maintain(__x, false);
        else
          s_insert(__x->m_right, __x, __v),
          sb_tree_maintain(__x, true);
      }
    }

    void
    s_success_erase(_Base_ptr __x)
    {
      _Link_type __y =  static_cast<_Link_type>(__x);
      if (__x->m_parent != &m_impl)
        while (__x->m_parent->m_parent != __x) {
          __x = __x->m_parent;
          __x->m_size -= 1;
        }
      delete __y;
    ...
    void
    s_erase(_Base_ptr& __x, const value_type& __v)
    {
      while (__x)
      {
  ...
  public :
    ...
    std::size_t
    rank(const value_type& __v) const
    {
      std::size_t __res = 0;
